...
<table style={{width: "100%", borderCollapse: "collapse", tableLayout: "fixed"}}>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.05)"}}>Mooncake backend: submit KV batches with <code>batch_transfer_async_write</code> and poll <code>get_batch_transfer_status</code> instead of blocking a worker thread per batch. Falls back to synchronous transfers on a Mooncake wheel without the async API.</td>
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.02)"}}><code>false</code></td>
    </tr>
    <tr>
