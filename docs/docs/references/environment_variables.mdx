...
<table style={{width: "100%", borderCollapse: "collapse", tableLayout: "fixed"}}>
  ...
  <tbody>
    ...
    <tr>
      <td style={{padding: "9px 12px", fontWeight: 500, backgroundColor: "rgba(255,255,255,0.02)"}}><code>SGLANG_HICACHE_FILE_BACKEND_IO</code></td>
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.05)"}}>I/O engine for file-based HiCache storage. <code>posix</code> keeps the backend's default client; <code>io_uring</code> selects the local io_uring client (<code>mooncake.uring</code>, O_DIRECT on a local NVMe file) for the <code>hf3fs</code> backend, equivalent to <code>{'{"client": "uring"}'}</code> in <code>--hicache-storage-backend-extra-config</code>.</td>
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.02)"}}><code>posix</code></td>
    </tr>
    <tr>
...
<table style={{width: "100%", borderCollapse: "collapse", tableLayout: "fixed"}}>
  ...
  <tbody>
    ...
    <tr>
      <td style={{padding: "9px 12px", fontWeight: 500, backgroundColor: "rgba(255,255,255,0.02)"}}><code>SGLANG_DISAGGREGATION_ASYNC_TRANSFER</code></td>
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.05)"}}>Mooncake backend: submit KV batches with <code>batch_transfer_async_write</code> and poll for completion (non-blocking <code>batch_transfer_poll</code> on wheels that have it, else <code>get_batch_transfer_status</code>) instead of blocking a worker thread per batch. Falls back to synchronous transfers on a Mooncake wheel without the async API.</td>
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.02)"}}><code>false</code></td>
    </tr>
    <tr>
...
<table style={{width: "100%", borderCollapse: "collapse", tableLayout: "fixed"}}>
  ...
  <tbody>
    ...
    <tr>
      <td style={{padding: "9px 12px", fontWeight: 500, backgroundColor: "rgba(255,255,255,0.02)"}}><code>MC_FORCE_TCP</code></td>
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.05)"}}>Make the Mooncake transfer engine install its TCP transport instead of RDMA (set by <code>--disaggregation-transfer-backend mooncake_tcp</code>). Mooncake keys on the variable being present, so unset it rather than setting it to <code>false</code>.</td>
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.02)"}}><code>false</code></td>
    </tr>
    <tr>
      <td style={{padding: "9px 12px", fontWeight: 500, backgroundColor: "rgba(255,255,255,0.02)"}}><code>MC_TCP_IO_BACKEND</code></td>
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.05)"}}>Data plane of the Mooncake TCP transport: <code>asio</code> or <code>io_uring</code>. Wire-compatible with asio peers; falls back to asio when io_uring is unavailable (old kernel, seccomp).</td>
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.02)"}}>Not set (<code>asio</code>)</td>
    </tr>
    <tr>
      <td style={{padding: "9px 12px", fontWeight: 500, backgroundColor: "rgba(255,255,255,0.02)"}}><code>MC_TCP_ZC</code></td>
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.05)"}}>Enable dmabuf zero copy on the Mooncake TCP transport (io_uring zero-copy receive into GPU memory, devmem TCP transmit). Needs kernel 6.16+ and a header/data-split NIC.</td>
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.02)"}}><code>false</code></td>
    </tr>
    <tr>
      <td style={{padding: "9px 12px", fontWeight: 500, backgroundColor: "rgba(255,255,255,0.02)"}}><code>MC_DPDK_PORTS</code></td>
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.05)"}}>DPDK ports (PCI addresses, or <code>net_af_xdp,iface=...</code>) for the Mooncake DPDK transport; its presence enables the transport (<code>--disaggregation-transfer-backend mooncake_dpdk</code> selects it).</td>
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.02)"}}>Not set</td>
    </tr>
    <tr>
