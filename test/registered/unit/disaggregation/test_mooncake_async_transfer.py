...
``_transfer_data`` submit + poll loop with its sleep backoff, the outstanding
counter the unified-memory move gate reads, and the fallback to synchronous
transfers when the installed wheel has no async API.
"""
...
class _FakeTransferEngine:
    ...
    def __init__(self, statuses=(0,), submit_id=7, supports=True):
        self.statuses = _StatusSequence(statuses)
        self.submit_id = submit_id
